...
export async function requestSimpleCommand(ctx: RequestCommandContext): Promise<void> {
  if (!ctx.from) {
...
<i>Пример: "Хочу татуировку дракона в стиле реализм на плече, размер средний, бюджет до 50000 рублей"</i>`;
  ...
  // Сохраняем информацию о том, что пользователь начал создание запроса
  if (!(ctx as any).session) {
    (ctx as any).session = {};
  }
  (ctx as any).session[`request_${userId}`] = {
  ...
  console.log('Simple request command initialized for user:', userId);
  console.log('Session saved:', (ctx as any).session[`request_${userId}`]);
}

export async function handleSimpleRequestText(ctx: RequestCommandContext): Promise<void> {
  ...
  if (session && session.waitingForDescription) {
    ...
    try {
      ...
      if (ctx.database) {
        ...
        await sendRequestToMasterSimple(ctx, session.userInfo, requestId, description);
...
async function sendRequestToMasterSimple(ctx: RequestCommandContext, userInfo: any, requestId: number, description: string): Promise<void> {
